...
import { List, Icon, Typography, Avatar, Button, Skeleton, Row, Col } from 'antd';
...
export default () => {
    ...
    const getMoreData = () => {
        setLoading(true);
        setData(data.concat(
            [...new Array(_COUNT_)].map(
                e => ({ loading: true, name: {}, gender: {} })
            )
        ));
        getData((results) => {
            setTimeout(() => {
    ...
    return (
        <div className="demo-wrapper">
            <List
                            ...
                            <Skeleton title={false} avatar loading={item.loading} active>
                                <List.Item.Meta
                                    ...
                                    description={
                                        <div>
                                            <Row gutter={10}>
                                                <Col xs={24} lg={12}>Gender:</Col>
                                                <Col xs={24} lg={12}>{item.gender}</Col>
                                            </Row>
                                        </div>
                                    }
                                />
                ...
                style={{ width: 500 }}
                loading={initLoading}
                loadMore={loading || initLoading ? null :
